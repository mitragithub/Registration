...
%             keyboard
        ...
        % in human there are some bad slices
        if contains(target_dir,'human')
            if contains(files{i},'_0584') || contains(files{i},'_0869') || contains(files{i},'_0890') || contains(files{i},'_1913') ...
                    || contains(files{i},'_0881')|| contains(files{i},'_0419')                
                W_ = W_*0;
            end
...
%             val(i) = normcdf((val(i) - Jmean)/Jstd);
    ...
    % we use laplacian in fourier domain
    laplacianpower = 1;
    npad = 3*r;
    ntot = n+npad*2;
    LLhat = (-2.0*r^2*( cos( 2.0*pi*(0:ntot-1)/ntot ) - 1 )/1^2).^laplacianpower;
    LLhat = reshape(LLhat,1,1,length(LLhat));
...
%         I = I ./ W_;
        ...
        if it == 1
            I = I0.*W0;
            Ihat = fft(padarray(I,[0,0,npad],'both','symmetric'),[],3);
            niterI = 100;
        else
            niterI = 5;
        end
        ...
        for itI = 1 : niterI
            ...
            ImI0hat = fft(padarray((I - I0).*W0,[0,0,npad],'both','symmetric'),[],3);

            grad = (ImI0hat + Ihat.*LLhat*factor)./(1.0 + LLhat*factor);
            Ihat = Ihat - grad*eI;
        ...
        for i = 1 : n % loop over slices
            ...
            if any(isnan(step(:))) || any(isinf(step(:)))
                % for some slices the weight is 0 everywhere (bad slices)
                % we will get nans, just stay at identity
%                 keyboard
